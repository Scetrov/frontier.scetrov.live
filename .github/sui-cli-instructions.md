...
2. Build and write a genesis config (includes faucet):
...
3. (Optional) Inspect the generated files:
...
4. Use the `genesis-ceremony` helpers to list validators, add one, and finalize. Example
...
5. Build unsigned checkpoint, verify signoffs and finalize (minimal example):
...
6. Start the network using the written config directory:
...
2. Wait for Postgres to be ready (a few seconds). Then run `sui start` with indexer and
...
3. Verify the indexer connected and GraphQL is listening (default 0.0.0.0:9125):
