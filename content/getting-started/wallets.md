...
What this means is effectively each seed phrase (i.e. `need enter link blood addict year vicious vendor below soup invest crime`) will result in an entire hierarchy of wallets which while all derived from the same 128-bit (for a 12-word phrase) or 256-bit (for a 24-word phrase) number are not mathematically provable as being connected to each other.
